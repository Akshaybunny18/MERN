      ...
      <main className="max-w-7xl mx-auto px-4 py-8 animate-fade-in">
        ...
        {generatedCreds && (
          <div className="bg-green-500/10 border border-green-500/30 p-6 rounded-lg mb-8 relative">
            ...
            <button onClick={() => setGeneratedCreds(null)} className="absolute top-4 right-4 text-text-secondary hover:text-text-primary">✕</button>
          </div>
        )}
                  ...
                  {organizers.length === 0 ? (
                    ...
                    organizers.map(org => (
                      <tr key={org._id} className={`border-b border-glass-border/50 hover:bg-white/5 transition-colors ${org.isDisabled ? 'opacity-50' : ''}`}>
                        <td className="p-4">
                          <div className="font-bold text-text-primary">{org.organizerProfile?.organizerName || 'Unnamed'}</div>
                        ...
                        <td className="p-4 text-right">
                          <div className="flex justify-end gap-2">
                            <button onClick={() => handleToggleStatus(org._id)} className="p-2 bg-bg-secondary rounded hover:bg-text-primary/10 text-text-secondary hover:text-text-primary transition-colors" title={org.isDisabled ? "Enable Account" : "Disable Account"}>
      ...
      {showAddModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-fade-in">
          <div className="glass-panel w-full max-w-2xl p-6 relative">
            <button onClick={() => setShowAddModal(false)} className="absolute top-4 right-4 text-text-secondary hover:text-text-primary">✕</button>
            ...
            <form onSubmit={handleAddOrganizer} className="space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  ...
                  <input type="text" required value={newOrg.organizerName} onChange={e => setNewOrg({ ...newOrg, organizerName: e.target.value })} className="w-full bg-bg-secondary border border-glass-border rounded-lg px-3 py-2 text-text-primary focus:border-accent-neon" />
                </div>
                <div>
                  <label className="block text-xs text-text-secondary mb-1">Category</label>
                  <input type="text" required placeholder="e.g., Technical, Cultural" value={newOrg.category} onChange={e => setNewOrg({ ...newOrg, category: e.target.value })} className="w-full bg-bg-secondary border border-glass-border rounded-lg px-3 py-2 text-text-primary focus:border-accent-neon" />
                </div>
                <div className="md:col-span-2">
                  <label className="block text-xs text-text-secondary mb-1">Login Email (Required for login)</label>
                  <input type="email" required placeholder="club@iiit.ac.in" value={newOrg.email} onChange={e => setNewOrg({ ...newOrg, email: e.target.value })} className="w-full bg-bg-secondary border border-glass-border rounded-lg px-3 py-2 text-text-primary focus:border-accent-neon" />
                </div>
              </div>
